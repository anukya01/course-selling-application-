...
export const getStats = async (req, res) => {
  try {
    ...
    const purchases = await Purchase.find().populate("courseId").populate("userId");
    const totalRevenue = purchases.reduce((sum, p) => sum + (p.courseId?.price || 0), 0);
    const recentEnrollments = purchases.slice(-10).reverse().map((p) => ({
      student: p.userId?.firstName + " " + p.userId?.lastName,
      course: p.courseId?.title,
      amount: p.courseId?.price,
      date: p.createdAt,
    }));
    res.status(200).json({ totalCourses, totalStudents, totalRevenue, recentEnrollments });
  } catch (error) {
    res.status(500).json({ errors: "Error fetching stats" });
